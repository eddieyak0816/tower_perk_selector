...
def log_perk_selection(window_name, perk1_text, perk1_priority, perk2_text, perk2_priority, selected_perk,
    ...
    """Log a perk selection decision to the log file."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
# ============================================
# PERK PRIORITY LIST FOR "DADDY BLUESTACK" - Custom order
# ============================================
...
PERK_PRIORITY_DADDY = [
    ...
    (17, ["smart missiles"], []),                            # smart missiles
    (18, ["health regen"], []),                               # health regen (generic)
    (19, ["cash per wave"], []),                             # x12.00 cash per wave
    (20, ["boss health"], []),                               # boss health -70%, but boss speed +50%
    (21, ["ranged enemies", "attack distance"], []),         # ranged enemies attack distance reduced
    (22, ["life", "steal"], []),                             # lifesteal x2.50
    (23, ["enemies speed"], []),                             # enemies speed -40%
    (24, ["enemies have", "health"], ["max health", "health regen"]),  # enemies have -50% health
    (25, ["tower health regen"], []),                         # tower health regen x8
    (26, ["coins", "tower max health"], []),                 # x1.80 coins, but tower max health
    (27, ["tower damage", "bosses"], []),                    # x1.50 tower damage, but bosses
    (28, ["chain lightning"], []),                           # chain lightning (moved to #28)
    (29, ["inner land mines"], []),                          # inner land mines (moved to #29)
    (30, ["chrono field"], []),                              # chrono field (moved to #30)
    (31, ["spotlight"], []),                                 # spotlight (moved to #31)
    (32, ["black hole"], []),                                # black hole (moved to #32)
    (33, ["poison swamp"], []),                              # poison swamp (moved to #33)
    (34, ["swamp radius"], []),                              # swamp radius x1.5 (moved to #34)
]
...
def get_perk_priority(perk_text, window_name):
    """Get the priority of a perk based on keyword matching.
    Uses different priority lists for Daddy vs Maximus windows."""
    perk_text_lower = perk_text.lower()
    
    # Choose which priority list to use based on window name
    if "daddy" in window_name.lower():
        priority_list = PERK_PRIORITY_DADDY
    else:
        priority_list = PERK_PRIORITY_MAXIMUS
...
def main_loop():
    ...
    print("=" * 60)
    print("Tower Idle Defense - Perk Automator v6 (Daddy Custom)")
    print("=" * 60)
    print()
    ...
    # Log startup
    write_to_log("=" * 70)
    write_to_log("PERK AUTOMATOR STARTED - DADDY CUSTOM VERSION")
    write_to_log("=" * 70)
    ...
    print("  - DIFFERENT perk lists for Daddy vs Maximus windows")
    print()
