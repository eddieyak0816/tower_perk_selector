...
PERK_PRIORITY = [
    ...
    (22, ["cash per wave"], []),                             # x12.00 cash per wave
    (23, ["chain lightning"], []),                           # chain lightning
    (24, ["smart missiles"], []),                            # smart missiles
    (25, ["inner land mines"], []),                          # inner land mines
    (26, ["boss health"], []),                               # boss health -70%, but boss speed +50%
    (27, ["ranged enemies", "attack distance"], []),         # ranged enemies attack distance reduced
    (28, ["life", "steal"], []),                             # lifesteal x2.50
    (29, ["enemies speed"], []),                             # enemies speed -40%
    (30, ["enemies have", "health"], ["max health", "health regen"]),  # enemies have -50% health
    (31, ["health regen"], []),                              # tower health regen x8
    (32, ["coins", "tower max health"], []),                 # x1.80 coins, but tower max health
    (33, ["tower damage", "bosses"], []),                    # x1.50 tower damage, but bosses
]
...
def is_purple_background(window_name, perk_region):
    ...
    if purple_bg_dist <= PURPLE_TOLERANCE:
        print(f"  [{window_name}] -> PURPLE BACKGROUND detected (distance: {purple_bg_dist})")
        return True, pixel
    ...
    is_dark_purple = (
        ...
        g < 30 and  # Green is very low (characteristic of this purple)
...
def main_loop():
    ...
    while True:
        try:
            ...
            # Check each window for new perks
            for window_name in WINDOWS:
                check_failsafe()
